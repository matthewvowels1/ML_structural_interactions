...
def estimate_skeleton(indep_i_test_func, indep_ci_test_func, data_matrix, alpha, num_nearest=5, **kwargs):
    """Estimate a skeleton graph from the statistis information.
    Args:
        indep_i/ci_test_func: the functions for CI and independency test.
        data_matrix: data (as a numpy array).
        alpha: the significance level.
        num_nearest: number of nearest neighbours for knn MI and CMI tests
        kwargs:
        ...
        for (i, j) in permutations(node_ids, 2):
            ...
            if len(adj_i) >= l:
                ...
                for k in combinations(adj_i, l):
                    ...
                    s = set(k)
                    if (len(s) == 0):
                        p_val = indep_i_test_func(data_matrix, i, j, k=num_nearest)  # here k is num neighbours
                    else:
                        p_val = indep_ci_test_func(data_matrix, i, j, s=set(k), k=num_nearest)  # here k is num neighbours
